...
Brazil currently has 9 supercomputers on the TOP500 list (top500.org). The 500 most potent non-distributed computer systems in existence are ranked and described in detail by the TOP500 project. The last time LNCC's [SDumont](https://sdumont.lncc.br/) was included was at position 462 on the list from November 2022. It is no longer on the list.
...
[SDumont][1] is a Brazilian public supercomputer dedicated to teaching and research, and is the main machine of [SINAPAD][2]. The manufacturer is the French company [Atos/Bull][3] and has an installed processing capacity of around [5.1 petaflops peak][4], divided into [8 partitions][5]: 4 partitions with CPU, 3 partitions with CPU + GPU, and 1 partition with CPU + Xeon Phi. SDumont should rank in the TOP 500 with this capacity, however because it is partitioned, it can only run [Linpack Benchmark][6] on a single partition, with only [one partition taken into account](7) by the TOP500. A [new upgrade is planned for 2024][8], boosting its capacity to 23 petaflops.
